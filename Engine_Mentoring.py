...
st.subheader("Prompt ID: " + data_obj['PROMPT_ID'])
...
with st.container(border=True):
    ...
    for j, response in enumerate(data_obj['RESPONSES']):
        st.write("Response " + str(j) + ": " + response)
#     response_feedback[response] = st.slider("How would you rate this response?", 0, 7, key="response_" + str(j))

with st.container(border=True):
    st.subheader("Feedback")
    st.write("Please provide your feedback in the google form. Be sure to copy the item_id from above!")
